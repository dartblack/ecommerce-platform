...
if [ ! -d /var/www/html/vendor ] || [ -z "$(ls -A /var/www/html/vendor 2>/dev/null)" ]; then
    echo "Queue Worker: Installing Composer dependencies..."
    composer install --no-interaction --prefer-dist --optimize-autoloader
    chown -R www-data:www-data /var/www/html/vendor 2>/dev/null || true
else
    echo "Queue Worker: Composer dependencies already installed, skipping..."
fi
...
# Switch to www-data user for running the queue worker
